...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT
      ...
      num_bus1,
      company_bus1,
      end_stop AS transfer_stop,
      num_bus2,
      company_bus2
    FROM
      (SELECT
        stopb.name AS end_stop,
        a.company AS company_bus1,
        a.num AS num_bus1
      FROM
        routes a
      JOIN
      ...
      JOIN
        stops stopb ON (b.stop_id = stopb.id)
      WHERE
        stopa.name = 'Craiglockhart') AS routes_from_Craiglockhart
    INNER JOIN
      (SELECT
        stopa.name AS start_stop,
        a.company AS company_bus2,
        a.num AS num_bus2
      FROM
        routes a
      JOIN
      ...
      JOIN
        stops stopb ON (b.stop_id = stopb.id)
      WHERE
        stopb.name = 'Sighthill') AS routes_to_Sighthill
    ON routes_from_Craiglockhart.end_stop = routes_to_Sighthill.start_stop;
  SQL
end
